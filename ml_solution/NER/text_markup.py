...
from natasha import NamesExtractor, AddrExtractor, DatesExtractor, MoneyExtractor, MorphVocab, NewsNERTagger, \
    NewsMorphTagger, NewsSyntaxParser
from natasha import Doc, Segmenter, NewsEmbedding, Doc
...
class TextMarkUp:
    ...
    def get_markup(self, text: str) -> List[MarkUpBlock]:
        ...
        if self._is_bert:
            ...
            # text_markup = self.rebuild_markup(self.get_natasha_markup(text=text))
    ...
    def get_name_markup(self, text_markup: List[MarkUpBlock]) -> List[MarkUpBlock]:
        ...
        for tm in range(len(text_markup)):
            if text_markup[tm].block_type == MarkUpType.NOTHING:
                dates = self._names_extractor(text_markup[tm].text)
                increment = text_markup[tm].start
                left_bounce = 0
                for date in dates:
                    markup = {}
                    if date.as_json["fact"].first is not None and len(date.as_json["fact"].first) > 1:
                        markup["First"] = date.as_json["fact"].first
                    if date.as_json["fact"].last is not None and len(date.as_json["fact"].last) > 1:
                        markup["Last"] = date.as_json["fact"].last
                    if date.as_json["fact"].middle is not None and len(date.as_json["fact"].middle) > 1:
                        markup["Middle"] = date.as_json["fact"].middle

                    if len(markup) < 3 :
                        continue
                    if date.as_json["start"] > 0:
                        result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce: date.as_json["start"] + 1],
                                                         block_type=MarkUpType.NOTHING,
                                                         start=increment + left_bounce,
                                                         end=increment + left_bounce + date.as_json["start"],
                                                         attachments=markup))

                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[date.as_json["start"]:
                                                                               date.as_json["stop"]],
                                                     block_type=MarkUpType.PERSON,
    ...
    def get_money_markup(self, text_markup: List[MarkUpBlock]) -> List[MarkUpBlock]:
        """
        This method receives the pre-marked text as input and places the monetary values from the pieces
        that have not yet been marked up.
        :param text_markup: Pre-marked text
        ...
        for tm in text_markup:
            if tm.block_type == MarkUpType.NOTHING:
                dates = self._money_extractor(tm.text)
                increment = tm.start
                left_bounce = 0

                for date in dates:
                    if date.as_json["start"] > 0:
                        result_markup.append(MarkUpBlock(text=tm.text[left_bounce: date.as_json["start"]],
                                                         block_type=MarkUpType.NOTHING,
                                                         start=increment + left_bounce,
                                                         end=increment + left_bounce + date.as_json["start"]))
                        ...
                        result_markup.append(MarkUpBlock(text=tm.text[date.as_json["start"]: date.as_json["stop"]],
                                                         block_type=MarkUpType.MONEY,
                                                         start=increment + left_bounce + date.as_json["start"],
                                                         end=increment + left_bounce + date.as_json["stop"],
                                                         attachments=markup))
                ...
                if tm.end - (increment + left_bounce) > 0:
                    result_markup.append(MarkUpBlock(text=tm.text[left_bounce: tm.end - increment],
                                                     block_type=MarkUpType.NOTHING,
                                                     start=increment + left_bounce,
                                                     end=tm.end))
            else:
                result_markup.append(tm)
    ...
    def get_rs_markup(self, text_markup: List[MarkUpBlock]) -> List[MarkUpBlock]:
        """
        This class receives the pre-marked text as input and places the phones from the pieces
         that have not yet been marked up.
        :param text_markup: Pre-marked text
        ...
        for tm in range(len(text_markup)):
            if text_markup[tm].block_type == MarkUpType.NOTHING:
                ...
                for rs in rss:
                    if rs["start"] > 0:
                        result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce: rs["start"]],
                                                         block_type=MarkUpType.NOTHING,
                                                         start=increment + left_bounce,
                                                         end=increment + left_bounce + rs["start"]))
                    markup = {}
                    if "RS" in rs["fact"]:
                        markup["RS"] = rs["fact"]["RS"]
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[rs["start"]:
                                                                               rs["stop"]],
                                                     block_type=MarkUpType.RS,
                                                     start=increment + left_bounce + rs["start"],
                                                     end=increment + left_bounce + rs["stop"],
                                                     attachments=markup))
                    left_bounce = rs["stop"]
                if text_markup[tm].end - (increment + left_bounce) > 0:
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce:
    ...
    def get_ls_markup(self, text_markup: List[MarkUpBlock]) -> List[MarkUpBlock]:
        """
        This class receives the pre-marked text as input and places the phones from the pieces
         that have not yet been marked up.
        :param text_markup: Pre-marked text
        ...
        for tm in range(len(text_markup)):
            if text_markup[tm].block_type == MarkUpType.NOTHING:
                ...
                for ls in rss:
                    if ls["start"] > 0:
                        result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce: ls["start"]],
                                                         block_type=MarkUpType.NOTHING,
                                                         start=increment + left_bounce,
                                                         end=increment + left_bounce + ls["start"]))
                    markup = {}
                    if "LS" in ls["fact"]:
                        markup["LS"] = ls["fact"]["LS"]
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[ls["start"]:
                                                                               ls["stop"]],
                                                     block_type=MarkUpType.LS,
                                                     start=increment + left_bounce + ls["start"],
                                                     end=increment + left_bounce + ls["stop"],
                                                     attachments=markup))
                    left_bounce = ls["stop"]
                if text_markup[tm].end - (increment + left_bounce) > 0:
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce:
    ...
    def get_index_markup(self, text_markup: List[MarkUpBlock]) -> List[MarkUpBlock]:
        """
        This class receives the pre-marked text as input and places the phones from the pieces
         that have not yet been marked up.
        :param text_markup: Pre-marked text
        ...
        for tm in range(len(text_markup)):
            if text_markup[tm].block_type == MarkUpType.NOTHING:
                indexes = TextMarkUp._index_extractor(text_markup[tm].text)
                increment = text_markup[tm].start
                left_bounce = 0
                for index in indexes:
                    if index["start"] > 0:
                        result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce: index["start"]],
                                                         block_type=MarkUpType.NOTHING,
                                                         start=increment + left_bounce,
                                                         end=increment + left_bounce + index["start"]))
                    markup = {}
                    if "INDEX" in index["fact"]:
                        markup["INDEX"] = index["fact"]["INDEX"]
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[index["start"]:
                                                                               index["stop"]],
                                                     block_type=MarkUpType.INDEX,
                                                     start=increment + left_bounce + index["start"],
                                                     end=increment + left_bounce + index["stop"],
                                                     attachments=markup))
                    left_bounce = index["stop"]
                if text_markup[tm].end - (increment + left_bounce) > 0:
                    result_markup.append(MarkUpBlock(text=text_markup[tm].text[left_bounce:
                                                                               text_markup[tm].end - increment],
                                                     block_type=MarkUpType.NOTHING,
            ...
            while index < len(text_markup):
                ...
                if text_markup[index].block_type != last_tag:
                    result.append(MarkUpBlock(text=text_markup[index].text,
                                              ...
                                              attachments=text_markup[index].attachments))
                else:
    ...
    @staticmethod
    def _RS_extractor(text: str) -> List[Dict[str, Any]] or None:
        """
        This is a generator that searches for bic's in the text.
        Supported:
        р/с XXXXXXXXXX
        ...
        re_RS = '(р/с).?.?\d{20}(\s|\D)?'
        text = f"{text} "
        while re.search(re_RS, text, re.IGNORECASE) is not None:
            RS = re.search(re_RS, text, re.IGNORECASE)
            yield {"start": RS.start() + left_bounce, "stop": RS.end() + left_bounce, "fact":
                {"BIC": TextMarkUp._clean_string(text[RS.start(): RS.end()]).strip()}}
            left_bounce = RS.end()
            text = text[RS.end():]

    @staticmethod
    def _LS_extractor(text: str) -> List[Dict[str, Any]] or None:
        """
        This is a generator that searches for bic's in the text.
        Supported:
        л/с XXXXXXXXXX
        ...
        while re.search(re_LS, text, re.IGNORECASE) is not None:
            ...
            yield {"start": LS.start() + left_bounce, "stop": LS.end() + left_bounce, "fact":
                {"BIC": TextMarkUp._clean_string(text[LS.start(): LS.end()]).strip()}}
            left_bounce = LS.end()
            text = text[LS.end():]

    @staticmethod
    def _index_extractor(text: str) -> List[Dict[str, Any]] or None:
        """
        This is a generator that searches for bic's in the text.
        Supported:
        л/с XXXXXXXXXX
        ...
        """
        left_bounce = 0
        re_index = '(индекс)? \d{6}.?(\s|\D)?'
        text = f"{text} "
        while re.search(re_index, text, re.IGNORECASE) is not None:
            index = re.search(re_index, text, re.IGNORECASE)
            yield {"start": index.start() + left_bounce, "stop": index.end() + left_bounce, "fact":
                {"BIC": TextMarkUp._clean_string(text[index.start(): index.end()]).strip()}}
            left_bounce = index.end()
            text = text[index.end():]
